...
## Description
`gateCounts` is an R package for calculating cumulative visitor counts, provided raw daily gate counts, gate directionality and gate counter maximum value. The package was developed for calculating cumulative gate counts in libraries. However, the methods can be applied to calculate cumulative visitor counts from any setting. 
...
Negative visitor counts can result from calculation if the gate counter has reset or if a lower gate count value has been entered compared to previous day. This package attempts to correct for both of these scenarios. 

The package attempts to account for when the daily gate count has been forgotten to be reported. This method doesn’t assign counts for missed days, but rather adjust for cumulative sum.
